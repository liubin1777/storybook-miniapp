...
program
  .command('add')
  .description('创建组件的 _storybook_ 测试环境')
  .action(callback);

function callback() {
  // 获取组件的目录名字
  const pathSegments = path.resolve('.').split('/');
  let componentNameEN = pathSegments[pathSegments.length-1];
  ...
  inquirer
    ...
    .then(() => {
      ...
      const destPath = path.resolve(`./_storybook_`);
      // 复制 _storybook_ 模板目录
      utils.copyTpl(originPath, destPath, tempContext, (e) => {
        e
          ? console.error(e)
          : console.log(`组件【${componentNameCN}】模板 _storybook_ 创建成功`);
      });
    })
    .then(() => {
      // 重新运行
